...
import { setImage } from "../../store/memberSlice.tsx";
...
const useChangeProfileImg = () => {
  ...
  const imgParams = (formData: FormData) => {
    const params: RawAxiosRequestConfig = {
      url: `${import.meta.env.VITE_BASE_URL}/member-service/auth/member/image`,
      method: "post",
      headers: {
  ...
  const changeImg = (formData: FormData) => {
    axios
      ...
      .then((response) => {
        ...
        dispatch(setImage(response.data.image_id));
        dispatch(setImageId(response.data.image_id));
      })
      .catch((error) => {
